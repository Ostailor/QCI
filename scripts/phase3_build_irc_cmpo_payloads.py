...
UNQUANTIZED_DIRECTORY = "unquantized_payloads"
MANIFEST_NAME = "payload_manifest.csv"
AUDIT_CSV_NAME = "coefficient_audit.csv"
AUDIT_MD_NAME = "coefficient_audit.md"
...
def _load_public_payloads(
    path: Path, pattern: str = "*.json"
...
def build_payloads(
    ...
    *,
    ...
    public_payloads = _load_public_payloads(
        _resolve(config["source_payload_dir"]),
        str(config.get("source_payload_glob", "*.json")),
    )
    ...
    audit_markdown = "\n".join(
        (
            "# IRC-CMPO Coefficient Audit",
            "",
            "This audit records the final six payloads used for the reported experiment.",
...
def main() -> None:
    ...
    parser.add_argument(
        "--surrogate-model",
        default="results/phase3/irc_cmpo/surrogate/model.json",
    )
    parser.add_argument("--output-dir")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the planned six-payload build without reading inputs or writing files.",
    )
    ...
    if args.dry_run:
        print(
            json.dumps(
                {
                    "dry_run": True,
                    "action": "build_irc_cmpo_payloads",
                    "config": args.config,
