...
def _execution_provenance_valid(row: dict[str, str]) -> bool:
    executed = _as_bool(row.get("qci_executed", False))
    build_time_value = row.get("build_time_qci_executed", row.get("qci_executed", False))
    if _as_bool(build_time_value):
        return False
    if not executed:
        return True
    try:
        repeats = int(float(row.get("qci_repeat_budget", "0")))
    except (TypeError, ValueError):
        return False
    return (
        str(row.get("qci_execution_status", "")).strip().lower() == "completed"
        and bool(str(row.get("qci_job_id", "")).strip())
        and repeats > 0
    )
...
def _payload_checks(path: Path, max_variables: int, max_degree: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    build_notice = next(
        (str(value) for key, value in payload.items() if str(key).endswith("_notice")),
        "",
    )
    _check(
        checks,
        "qci_not_mislabeled_at_build_time",
        sc.get("qci_executed", False) is False and "not submitted" in build_notice.lower(),
        build_notice,
    )
...
def validate(result_dir: Path, *, max_variables: int, max_degree: int, dry_run: bool) -> dict[str, Any]:
    ...
    executed_count = sum(_as_bool(row.get("qci_executed", False)) for row in manifest_rows)
    _check(
        checks,
        "manifest_execution_provenance",
        all(_execution_provenance_valid(row) for row in manifest_rows),
        f"completed_execution_rows={executed_count}; build-time execution flags must remain false",
    )
    ...
    if not dry_run:
        ...
        global_checks = [item for item in checks if ":" not in str(item["check"])]
        payload_check_count = len(checks) - len(global_checks)
        lines = [
            "# SC-CMPO Validation",
            "",
            f"Overall: **{'PASS' if ready else 'FAIL'}**",
            "",
            f"- Payloads: {len(payload_summaries)} across {len(families)} public benchmark families",
            f"- Maximum variables: {result['max_variables']} (limit {max_variables})",
            f"- Maximum degree: {result['max_degree']} (limit {max_degree})",
            f"- Completed QCi provenance rows: {executed_count}",
            f"- Payload-level checks: {payload_check_count} passed",
            "- Every payload passed bounds, normalization, public-input-only, no-random-grid, "
            "no-undocumented-synthetic-value, source-provenance, and challenge-stage checks.",
            "",
            "## Benchmark Summary",
            "",
        ...
        for benchmark in sorted(counts):
            rows = [row for row in payload_summaries if row["benchmark"] == benchmark]
            lines.append(
                f"| {benchmark} | {len(rows)} | {max(int(row['scenario_count']) for row in rows)} | "
                f"{max(int(row['variable_count']) for row in rows)} | {max(int(row['degree']) for row in rows)} |"
            )
        lines.extend(
            [
                "",
