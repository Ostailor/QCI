...
def _load_payloads(path: Path, pattern: str = "*.json") -> dict[str, dict[str, Any]]:
    payloads = {
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the planned true-recourse dataset build without running solvers or writing files.",
    )
    ...
    if args.dry_run:
        print(
            json.dumps(
                {
                    "dry_run": True,
                    "action": "build_irc_cmpo_true_recourse_dataset",
                    "config": args.config,
