...
DEFAULT_MANIFEST = Path("results/phase3/irc_cmpo/payload_manifest.csv")
DEFAULT_OUTPUT_DIRECTORY = "validation"
EXACT_JSON = "exact_validation.json"
EXACT_CSV = "exact_candidates.csv"
STOCHASTIC_JSON = "stochastic_validation.json"
STOCHASTIC_CSV = "stochastic_samples.csv"
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the offline validation plan without running solvers or writing artifacts.",
    )
    ...
    if args.dry_run:
        print(
            json.dumps(
                {
                    "dry_run": True,
                    "action": "validate_irc_cmpo_offline",
                    "config": args.config,
