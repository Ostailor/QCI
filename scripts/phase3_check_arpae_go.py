#!/usr/bin/env python
"""Download and inspect the retained ARPA-E GO Phase 3 benchmark slice."""
...
DEFAULT_REPORT_PATH = Path("results/phase3/reproduced/public_data/arpae_go_feasibility.md")
...
FINAL_CHALLENGE1_MEMBERS = {
    "Original_Dataset_Offline_Edition_2/Network_01O-020/case.inl",
    "Original_Dataset_Offline_Edition_2/Network_01O-020/case.rop",
    "Original_Dataset_Offline_Edition_2/Network_01O-020/scenario_1/case.con",
    "Original_Dataset_Offline_Edition_2/Network_01O-020/scenario_1/case.raw",
}
...
def _load_download_manifest_record(data_dir: Path, resource_key: str) -> DownloadRecord | None:
    ...
    for row in reversed(rows):
        ...
        try:
            ...
            bytes_downloaded = 0
        archive_present = local_path.is_file()
        return DownloadRecord(
            ...
            status=(
                f"manifest_{row.get('status', 'recorded')}"
                if archive_present
                else "manifest_only_archive_not_vendored"
            ),
            message=(
                row.get("message", "")
                if archive_present
                else "The exact extracted benchmark slice is retained; rerun the fetch command to download the public archive."
            ),
        )
    return None
...
def extract_archive(path: Path, data_dir: Path, *, overwrite: bool, dry_run: bool) -> Path | None:
    ...
    if lower_name.endswith(".zip"):
        with zipfile.ZipFile(path) as archive:
            ...
            if path.name == "Challenge_1_Original_Dataset_2_Scenarios.zip":
                members = [member for member in members if member.filename in FINAL_CHALLENGE1_MEMBERS]
                missing = FINAL_CHALLENGE1_MEMBERS - {member.filename for member in members}
                if missing:
                    raise ValueError(f"ARPA-E archive is missing the retained case files: {sorted(missing)}")
...
def run_check(args: argparse.Namespace) -> dict[str, Any]:
    ...
    initial_parseable = [
        path for path in initial_files if path.suffix.lower() in PARSEABLE_EXTENSIONS
    ]
    ...
    if (
        not initial_parseable
        and download_record is not None
        and download_record.local_path.exists()
        and not args.no_extract
        and extracted_dir is None
    ):
        extracted_dir = extract_archive(
            download_record.local_path,
            data_dir,
            overwrite=args.overwrite,
            dry_run=args.dry_run,
        )
