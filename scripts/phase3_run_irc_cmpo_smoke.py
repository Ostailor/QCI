...
DEFAULT_FINAL_SUMMARY = Path("results/phase3/irc_cmpo/preflight_summary.json")
DEFAULT_PAYLOAD_MANIFEST = Path("results/phase3/irc_cmpo/payload_manifest.csv")
...
def generate_approved_commands(
    ...
    *,
    ...
    for name in expected:
        ...
        output = shlex.quote(f"results/phase3/reproduced/irc_cmpo/smoke/{name}")
...
def evaluate_smoke_response(
    job: Mapping[str, Any], response: Mapping[str, Any]
) -> dict[str, Any]:
    """Evaluate native integer smoke samples without rounding or projection.

    Scalarized masters intentionally have no dollar budget.  Only legacy
    hard-budget payloads apply the exact-dollar gate; the absence of that field
    is never interpreted as an inferred budget.
    """

    from cmpo.qci_integer_adapter import native_integer_samples, validate_integer_response
    ...
    budget_applicable = isinstance(budget_constraint, Mapping)
    exact_budget_count: int | None = 0 if budget_applicable else None
    ...
    for sample in samples:
        ...
        if budget_applicable:
            selected_cost = math.fsum(
                ...
                if int(value) == 1
                and variable["physical_asset_key"] == asset["asset_key"]
            )
            budget_feasible = selected_cost <= float(
                budget_constraint["amount_dollars"]
            ) + 1e-9
            if budget_feasible:
                assert exact_budget_count is not None
    ...
    optimum_found = optimum is None or any(
        list(sample) == optimum["coordinates"] for sample in samples
    )
    competitive = optimum is None or bool(
        energies and min(energies) <= float(optimum["energy"]) + 1e-8
    )
    budget_gate = not budget_applicable or bool(exact_budget_count)
    passed = bool(
        validation.valid
        and validation.native_integer_in_domain_count == validation.native_sample_count
        and validation.projected_sample_count == 0
        and locally_feasible
        and budget_gate
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the smoke-test plan without reading artifacts, writing files, or contacting QCi.",
    )
    ...
    if args.dry_run:
        print(
            json.dumps(
                {
                    "dry_run": True,
                    "action": "plan_irc_cmpo_qci_smoke",
                    "config": args.config,
    ...
    def submit(job: Mapping[str, Any]) -> dict[str, Any]:
        ...
        evaluation = evaluate_smoke_response(job, raw_response)
        return {
            "qci_file": qci_file,
            "file_response": file_response,
            "job_body": job_body,
