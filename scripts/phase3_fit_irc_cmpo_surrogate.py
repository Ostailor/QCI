...
def adjacent_anchor_pairs_from_payloads(
    ...
    for payload_path in sorted(Path(path).glob(pattern)):
...
def physical_interactions(
    ...
    for left_anchor, right_anchor in adjacent_anchor_pairs_from_payloads(
        payload_dir, payload_glob
    ):
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the planned surrogate fit without loading data or writing model artifacts.",
    )
