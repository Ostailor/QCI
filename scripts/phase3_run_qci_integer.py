...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the complete request preview without writing files or contacting QCi.",
    )
    return parser
...
def main() -> int:
    ...
    if args.dry_run:
        if args.max_total_num_levels is None:
            raise SystemExit("--max-total-num-levels is required for a request preview")
        preview = _preview(args, qci_file, levels)
        print(json.dumps({"dry_run": True, "request_preview": preview}, indent=2))
        return 0
