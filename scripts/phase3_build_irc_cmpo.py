...
DEFAULT_BUDGET_MANIFEST = Path("results/phase3/irc_cmpo/inputs/budget_master_manifest.csv")
DEFAULT_MASTER_CANDIDATES = Path("results/phase3/irc_cmpo/inputs/master_comparison.csv")
DEFAULT_PROJECTED_CANDIDATES = Path("results/phase3/irc_cmpo/inputs/decoded_portfolios.csv")
...
def _historical_candidate_seeds(config: dict[str, Any], assets: tuple[Any, ...]) -> pd.DataFrame:
    ...
    classical = pd.read_csv(
        _resolve(config.get("source_master_candidates", DEFAULT_MASTER_CANDIDATES))
    )
    ...
    projected = pd.read_csv(
        _resolve(config.get("source_projected_qci_candidates", DEFAULT_PROJECTED_CANDIDATES))
    )
    projected = projected[
...
def build_irc_cmpo(
    ...
    *,
    ...
    budgets = pd.read_csv(
        _resolve(config.get("source_budget_manifest", DEFAULT_BUDGET_MANIFEST))
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the planned build without reading inputs or writing artifacts.",
    )
    ...
    if args.dry_run:
        print(
            json.dumps(
                {
                    "dry_run": True,
                    "action": "build_irc_cmpo_candidate_catalog",
                    "config": args.config,
