            ...
            if isinstance(item, str):
                ...
                if raw_path:
                    ...
                    if not path.is_absolute():
                        path = manifest_dir / path
                    paths[path.name] = path
    for payload_dir in (experiment_dir / "qci_payloads", experiment_dir / "payloads"):
...
def _payload_path_from_context(
    ...
    if raw:
        ...
        if path.is_file():
            return path
        portable_path = manifest_paths.get(path.name)
...
def _decode_projection_rows(
    *,
    ...
    """Preserve completed SC-CMPO vectors for classical projection."""
    ...
    for sample_index, vector in enumerate(solutions):
        ...
        rows.append(
            {
                "method_name": "Scenario-Coupled Consensus CMPO (QCi)",
                "dataset": dataset,
                "backend": "qci_dirac3_sc_cmpo",
                "scenario": scenario,
                "patch": patch_label,
                "job_id": _job_id(response),
                "status": _status(response),
...
def _summarize_projection_payloads(repeat_metrics: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
...
def decode_qci_experiment(
    *,
    ...
    for response_path in response_files:
        ...
        try:
            ...
            if is_sc_cmpo:
                ...
                rows, failures = _decode_projection_rows(
    ...
    if repeat_metrics.empty:
        ...
        payload_summary = _summarize_projection_payloads(repeat_metrics)
        best_solutions = (
