...
def test_sc_cmpo_qci_decode_preserves_raw_vector_for_projection(tmp_path: Path) -> None:
    payload = build_sc_cmpo_from_config("configs/phase3_sc_cmpo_case14.yaml")[0].payload
    payload_path = tmp_path / "qci_payloads" / "sc_payload.json"
    ...
    (repeat_dir / "request.json").write_text(
        json.dumps(
            {
                "payload_path": "/original/runner/results/phase3/sc_cmpo/qci_payloads/sc_payload.json",
...
def test_sc_cmpo_build_baseline_and_validation_pipeline(tmp_path: Path) -> None:
    ...
    manifest_path = output_dir / "payload_manifest.csv"
    with manifest_path.open(newline="", encoding="utf-8") as handle:
        manifest_rows = list(csv.DictReader(handle))
        fieldnames = list(manifest_rows[0])
    for column in (
        "build_time_qci_executed",
        "qci_execution_status",
        "qci_job_id",
        "qci_repeat_budget",
    ):
    ...
    for index, row in enumerate(manifest_rows):
        row.update(
            {
                "build_time_qci_executed": "False",
                "qci_executed": "True",
                "qci_execution_status": "completed",
                "qci_job_id": f"job-{index}",
                "qci_repeat_budget": "30",
            }
        )
    ...
    completed_validation = validator.validate(
        output_dir, max_variables=132, max_degree=3, dry_run=True
    )
    assert completed_validation["ready"] is True
    assert completed_validation["qci_was_run"] is True
