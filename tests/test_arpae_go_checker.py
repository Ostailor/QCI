...
from scripts.phase3_check_arpae_go import extract_archive
...
def test_final_arpae_archive_extracts_only_the_retained_case(tmp_path: Path) -> None:
    archive_path = tmp_path / "Challenge_1_Original_Dataset_2_Scenarios.zip"
    retained = {
        "Original_Dataset_Offline_Edition_2/Network_01O-020/case.inl": "inl",
        "Original_Dataset_Offline_Edition_2/Network_01O-020/case.rop": "rop",
        "Original_Dataset_Offline_Edition_2/Network_01O-020/scenario_1/case.con": "con",
        "Original_Dataset_Offline_Edition_2/Network_01O-020/scenario_1/case.raw": "raw",
    }
    with zipfile.ZipFile(archive_path, "w") as archive:
        for name, value in retained.items():
            archive.writestr(name, value)
        archive.writestr(
            "Original_Dataset_Offline_Edition_2/Network_99O-999/scenario_1/case.raw",
            "unrelated",
        )

    output = extract_archive(archive_path, tmp_path / "data", overwrite=False, dry_run=False)

    assert output is not None
    assert len([path for path in output.rglob("*") if path.is_file()]) == 4
    assert not any("Network_99O-999" in path.as_posix() for path in output.rglob("*"))
