...
CATALOG = ROOT / "results/phase3/irc_cmpo/inputs/public_asset_catalog.csv"
...
def test_cubic_surrogate_interactions_record_physical_rationale(tmp_path: Path) -> None:
    fitter = _load_script("phase3_fit_irc_cmpo_surrogate")
    pairs, cubics, metadata = fitter.physical_interactions(_toy_assets(), tmp_path)
    assert pairs
    assert cubics
    for cubic in cubics:
        record = metadata[cubic]
        assert len(record["anchor_nodes"]) == 1
        assert record["interaction_class"] == "same_anchor_three_technology"
